...
export default async function DashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="font-heading text-2xl font-semibold">Zarzadzanie kontami</h1>
      ...
      <div className="flex flex-col gap-3 md:hidden">
        {sorted.map((user) => {
          const canManage = user.id !== actor.id && canManageAccount(actor, user);
          return (
            <Card key={user.id}>
              <CardHeader>
                <CardTitle className="text-sm break-all">{user.email}</CardTitle>
                <CardDescription>
              ...
              <CardContent className="flex flex-col gap-3">
                <Badge variant={STATUS_BADGE_VARIANT[user.status]} className="w-fit">
                  {statusLabels[user.status]}
                </Badge>
                <AccountActions
                  userId={user.id}
