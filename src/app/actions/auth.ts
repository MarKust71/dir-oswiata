...
export async function loginAction(
  ...
  if (user.status === AccountStatus.DISABLED) {
    return {
      message: "To konto zostalo dezaktywowane. Skontaktuj sie z administratorem.",
    };
  }
...
export async function verifyEmailAction(token: string) {
  ...
  if (record.expiresAt < new Date()) {
    ...
    return {
      success: false as const,
      message: "Link weryfikacyjny wygasl. Zarejestruj sie ponownie lub popros o nowy link.",
    };
  }
...
export async function resendVerificationAction(email: string) {
  ...
  return {
    message: "Jesli konto oczekuje na potwierdzenie e-mail, wyslalismy nowy link.",
  };
}
